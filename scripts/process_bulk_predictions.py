...
if args.model == 'aug':
    ...
    for i in range(1, 11):
        ...
        sub_df.to_csv(f'../data/PRJNA28331_filtered_proteins_positive_results_aug_pt{i}.csv', index=False)
        sub_headers, sub_seqs = sub_df['header'].values.tolist(), sub_df['seq'].values.tolist()
        SaveFastaFile(f'../data/PRJNA28331_filtered_proteins_positive_results_aug_pt{i}.fasta', sub_headers, sub_seqs)
    ...
    pos_df.to_csv('../data/PRJNA28331_filtered_proteins_positive_results_base.csv', index=False)
    pos_headers, pos_seqs = pos_df['header'].values.tolist(), pos_df['seq'].values.tolist()
    SaveFastaFile('../data/PRJNA28331_filtered_proteins_positive_results_base.fasta', pos_headers,
                  pos_seqs)
else:

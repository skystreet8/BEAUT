...
neg_headers, _ = ReadFastaFile('../data/negative_seqs_v2_wosimseqs_substrate_pocket_aug.fasta')
aug_headers, _ = ReadFastaFile('../data/substrate_pocket_sim_aug.fasta')
...
for k in neg_sample_headers:
    data.append([k, 0])
# For negative samples which were not selected, add them as test samples
...
# Do not use any selected negative samples for testing since there are already a huge many of negative test samples
num_neg_test = 0
# Randomly select 20% positive samples for testing
test_pos_indexes = pos_indexes[:num_pos_test]
# Test samples include positive test samples and all other negative samples which were not selected
test_indexes = test_pos_indexes + [i for i in range(len(pos_headers) + len(neg_sample_headers), len(df))]
# Calculate the numbers of positive & negative samples in each fold
pos_increment = (num_pos_seqs - len(test_pos_indexes)) // NUM_FOLDS
neg_increment = num_neg_seqs // NUM_FOLDS
print(pos_increment)
print(neg_increment)
# Assign positive & negative samples to each fold
...
# Add augmentation data only as training samples
df = pd.concat([df, aug_df], ignore_index=True)
df.to_csv(f'../data/sequence_dataset_v2_substrate_pocket_aug_train_only.csv', index=False)

...
components = [list(clu) for clu in components]
...
num_test_pos_headers = int(round(len(pos_headers) * TEST_RATIO, 0))
...
neg_h2s = {h: s for h, s in zip(neg_headers, neg_seqs)}
qdf = df.query('label == 0 and dataset_fold_1 == "test"')
test_neg_hs = qdf['header'].values.tolist()
test_neg_ss = [neg_h2s[h] for h in test_neg_hs]
qdf = df.query('label == 0 and dataset_fold_1 != "test"')
non_test_neg_hs = qdf['header'].values.tolist()
non_test_neg_ss = [neg_h2s[h] for h in non_test_neg_hs]
SaveFastaFile('../data/test_set_neg_all.fasta', test_neg_hs, test_neg_ss)
SaveFastaFile('../data/non_test_set_neg_all.fasta', non_test_neg_hs, non_test_neg_ss)
